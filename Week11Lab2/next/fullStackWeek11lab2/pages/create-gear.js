...
export default function CreateGearComponent() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const response = await fetch('http://localhost:8000/createGear', {
      method: 'POST',
    ...
    <div className={styles.container}>
      <div className={styles.formCard}>
        <div className={styles.header}>
          ...
          <h2 className={styles.title}>Add Gear Item</h2>
          <p className={styles.subtitle}>Fill in the gear item details below</p>
        </div>
        <form onSubmit={handleSubmit}>
          ...
          {showSuccess && <div className={styles.successMessage}>✓ Gear item saved successfully!</div>}
        </form>
      </div>
    </div>
  );
